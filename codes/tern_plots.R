...
head(tern_df_soil)
####
# Replace empty with Other if needed ######
#tern_df$Genus[tern_df$Genus=="Achlya"] <- "Other"
...
ggtern(data = tern_df_soil, 
  ...
  geom_point(aes(fill = Genus), 
             shape = 21,
             size = 2.5, position= position_jitter_tern(x=0.01, y=0.01, z=0.01)) +
  ...
  facet_wrap(vars(Order), ncol = 2, nrow = 4) +
