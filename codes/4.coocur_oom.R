...
row.names(site_by_species) <- site_by_species$X; site_by_species$X <- NULL
    ...
    if(randsummary == FALSE){  
      p <- ggplot(df.lower, aes(X1, X2)) + 
        geom_tile(aes(fill = factor(value,
                                    levels=c(-1,0,1))), 
                                    colour ="white") 
      p <- p + scale_fill_manual(values = c("#FFCC19","dark gray","light blue"), 
                                 name = "", labels = c("negative","random","positive"),
                                 drop=FALSE) + 
        ...
        xlab("") + ylab("") + 
        scale_x_discrete(limits=meas, expand = c(0.1, 0),drop=FALSE) + 
        scale_y_discrete(limits=meas, expand = c(0.1, 0),drop=FALSE) 
      ...
      p <- ggplot(df.lower, aes(X1, X2)) + 
        geom_tile(aes(fill = factor(value,levels=c(-1,0,1,-2))), 
                  colour ="white") 
      p <- p + scale_fill_manual(values = c("#FFCC19","dark gray","light blue","light gray"), 
                                 name = "", 
                                 labels = c("negative","random","positive","random"),
                                 drop=FALSE) + 
        theme(axis.text.x = element_blank(),
              axis.text.y = element_blank(),
              axis.ticks = element_blank(),
...
obs.v.exp <-
  function(mod){
    ...
    p <- ggplot(ptab, aes(x=exp_cooccur, 
                          y=obs_cooccur)) + 
      geom_point(aes(fill=factor(signs,
                                 levels=c(-1,0,1))), 
                 colour="black",
                 pch=21, 
                 size=2)
    ...
    p <- p + geom_abline(color="dark gray")
    p
  }

##############################################################

OBS_exp <- obs.v.exp(cooccur.oom)
OBS_exp

pair <- pair.profile(cooccur.oom)
